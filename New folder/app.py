...
A modern, production-grade web application for automated candidate screening, scoring, ranking, AI insights, and an Interactive AI Recruiter Copilot.
"""
...
from config import (
    GROQ_API_KEY, OPENAI_API_KEY, MODEL_PROVIDER, GROQ_MODEL,
...
# Custom CSS for Modern Aesthetic & Dark/Light Mode Polish
CUSTOM_CSS = """
<style>
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": "👋 Hello! I am your **AI Recruiter Copilot**. You can ask me to evaluate candidates, shortlist applicants, look up resume details, or schedule interviews!"}
    ]
...
with st.sidebar:
    ...
    api_key_input = st.text_input("Groq API Key", value=GROQ_API_KEY, type="password", help="Enter your Groq or OpenAI API key for LLM-powered insights.")
    active_key = api_key_input.strip() or GROQ_API_KEY

    if active_key:
        st.success("✅ Groq API Key Configured")
    else:
        st.info("ℹ️ Using Rule-based NLP fallback (No API key provided)")

    st.markdown("---")
    st.subheader("Demo Mode")
    if st.button("⚡ Load Demo Dataset", type="secondary", use_container_width=True):
        # Load sample JD
        ...
        # Screen sample resumes automatically
...
<div class="header-container">
    <h1 class="header-title">🎯 AI Resume Screening Agent & Copilot</h1>
    <p class="header-subtitle">Automated Multi-Format Screening, NLP Semantic Matching & Conversational AI Assistant</p>
</div>
...
with tab_upload:
    ...
    with col2:
        ...
        st.info("💡 Supports screening 10 to 50+ resumes simultaneously. Try the '⚡ Load Demo Dataset' button in the sidebar for instant testing!")
...
with tab_details:
    ...
    if not st.session_state.is_screened or not st.session_state.screening_results:
        ...
        with col_right:
            st.markdown("### 🤖 Candidate Assistant Tools")
            ...
            with llm_tab1:
                ...
                if st.button("Generate Executive Score Summary", key="btn_explain"):
                    with st.spinner("Generating score explanation..."):
                        explanation = recommender.explain_score(cand_report["candidate_name"], cand_report)
                        st.info(explanation)

            with llm_tab2:
                st.markdown("#### AI-Generated Interview Kit")
                if st.button("Generate Interview Questions", key="btn_questions"):
                    with st.spinner("Tailoring interview questions based on candidate profile..."):
            ...
            with llm_tab3:
                ...
                if st.button("Analyze Skill Gaps", key="btn_gaps"):
                    with st.spinner("Analyzing missing skill impact..."):
...
# ==========================================
# TAB 5: INTERACTIVE AI RECRUITER COPILOT
# ==========================================
with tab_agent:
    st.subheader("💬 Interactive AI Recruiter Copilot")
    st.markdown("Ask natural language queries, instruct the AI agent to shortlist applicants, schedule interviews, read specific resume details, or save notes to the candidate database.")

    if not st.session_state.is_screened or not st.session_state.screening_results:
        st.warning("⚠️ Please screen candidates or click '⚡ Load Demo Dataset' in the sidebar to activate the AI Copilot context.")
    else:
        agent = RecruiterAIAgent(api_key=active_key)

        # Preset Shortcut Action Buttons
        ...
        # User Input Handling
        user_input = st.chat_input("Type your question or instruction for the AI Copilot...") or selected_prompt
        ...
        if user_input:
            ...
            with st.chat_message("assistant"):
                with st.spinner("🤖 Thinking & executing actions..."):
                    ...
                    # Display Tool Actions if executed
        ...
        with st.expander("💾 Candidate Tracking Database Records (SQLite)"):
            ...
            if db_records:
                ...
                st.info("No saved candidate database records yet. Tell the AI Copilot e.g. 'Shortlist Alex Rivera' to save status!")
